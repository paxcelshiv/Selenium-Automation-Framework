...
public class MethodType {
	...
	public void findMethod(String methodType, String objectLocators,
		...
		try {
			...
			MainTestNG.LOGGER.info("exception occured in finding methods, method name is incorrect"
							+ e);
		} catch (Exception e) {
	...
	public void findElementByXpath(String objectLocators) {
		WebDriverWait wait = new WebDriverWait(WebDriverClass.getDriver(), 90);
	...
	public void compareValues(MethodParameters model){
    	try{
    		...
    		MainTestNG.LOGGER.info("inside campareValues()" + "values are=="
    				+ list1 + "data from excel="
    				+ lsitOfValues);
    		wait1(2000);
	...
	public void compareTwoListValues(MethodParameters model){
    	...
    	public void Demoschecked(MethodParameters model){
        	...
        	System.out.println(list.size());
        		String lsitOfValues=model.getData();
        		MainTestNG.LOGGER.info("inside Demoschecked()" + "values are=="
        		+ list + "data from excel="+ lsitOfValues);
        		wait1(2000);
        		String [] mylist=lsitOfValues.split("<%==>");
        		for(String str:mylist){
        			for(int i=0;i<list.size();i++){
        				if(str.trim().equalsIgnoreCase(list.get(i).getText().trim())){
        					WebDriverClass.getDriver().findElement(By.xpath(".//*[@id='kendoDemoGroups']/div[2]/table/tbody/descendant::span[text()='"+str.trim()+"']/parent::td/parent::tr/descendant::input")).click();
        				break;
        				}
        			}
        		  }
        	    MainTestNG.LOGGER.info("All values are matched with Demoschecked()");
            	}catch(Exception ex){
        		System.out.println(ex.toString());
        	}
    }
    	public void checked_checkbox_newdemo(MethodParameters model){
        	...
        	//List<WebElement> list=WebDriverClass.getDriver().findElements(By.xpath(model.getObjectLocators()));
        		String lsitOfValues=model.getData();
        		MainTestNG.LOGGER.info("inside checked_checkbox_newdemo()" + "values are=="
        		 + "data from excel="+ lsitOfValues);
        		wait1(2000);
        		String [] mylist=lsitOfValues.split("<%==>");
        		for(String str:mylist){		
